...
Applies 0001_init.sql via migrate_sql once per session so SQL-touching
unit tests (test_uuidv7_sql.py) can query the uuidv7() function and
...
def _apply_init_schema(db):
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"migrate_sql failed in unit test setup:\n"
